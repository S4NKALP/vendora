...
export default function CompletedOrder({ orders = [], navigation: propNavigation }) {
  ...
  const handleLeaveReview = async (order) => {
    console.log('Order data for review:', order);
    ...
    try {
      setLoading(true);
      const firstItem = orderItems[0];
      console.log('First item for review:', firstItem);
      ...
      const product = await fetchProductById(productId);
      console.log('Fetched product for review:', product);
      ...
      console.error('Error in handleLeaveReview:', error);
      Alert.alert('Error', 'Failed to load product details. Please try again.');
    } finally {
      setLoading(false);
    }
  ...
  if (!orders || orders.length === 0) {
    return (
      <View className="flex-1 justify-center items-center py-10">
        <Text className="text-gray-500">No completed orders found</Text>
      </View>
    );
  }
          ...
          <View key={index} className="mb-6 border rounded-lg border-gray-200 overflow-hidden">
            <View className="bg-gray-100 p-3 border-b border-gray-200">
              ...
              <Text className="text-gray-500">
                ...
                Status: <Text className="text-green-600 font-medium capitalize">{order.status}</Text>
              </Text>
            </View>
